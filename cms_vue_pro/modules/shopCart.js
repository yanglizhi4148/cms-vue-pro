...
let shopCart={
    ...
    mutations:{
        ...
        addNum(state,prod){
            ...
            //有可能你添加完了，去了别的页面，此时只是在内存中有数据修改，没有保存到localStorage，所以需要保存
        },
        //添加元素
        addProd(state,prod){
    ...
    actions:{
        addProductOrNum(context,prod){//context和store对象具备一致的属性
            //判断该商品是否存在于数组中，如果存在调用追加数据改变
            let myProds=context.getters.getProds;
            let index=myProds.findIndex(function(p){
                return p.id==prod.id;
            });
            if(index !=-1){//如果查找对应的元素，就追加
                context.commit('addNum',{index,num:prod.num});//只能传递二个参数，后面一个可以用对象
            }else{
